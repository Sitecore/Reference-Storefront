...
namespace Sitecore.Reference.Storefront.Controllers
{
    ...
    [AllowAnonymous]
        ...
        public ActionResult OrderConfirmation([Bind(Prefix = StorefrontConstants.QueryStrings.ConfirmationId)] string confirmationId)
        {
            ...
            {
                order = new CommerceOrder() { Status = Commerce.Connect.DynamicsRetail.Texts.Pending };                               
            }
        ...
        public JsonResult GetCardPaymentAcceptUrl()
        {
            try
            {
                var response = this.PaymentManager.GetPaymentServiceUrl();
                var result = new CardPaymentAcceptUrlJsonResult(response.ServiceProviderResult);
                return Json(result, JsonRequestBehavior.AllowGet);
            }
            catch (Sitecore.Commerce.OpenIDConnectionClosedUnexpectedlyException)
            {
            ...
            {
                CommerceLog.Current.Error("GetCardPaymentAcceptUrl", this, e);
                return Json(new BaseJsonResult("GetCardPaymentAcceptUrl", e), JsonRequestBehavior.AllowGet);
            }
        }      
        ...
        public JsonResult SetPaymentMethods(PaymentInputModel inputModel)
        {
            try
            {
                ...
                if (inputModel.FederatedPayment != null && !string.IsNullOrEmpty(inputModel.FederatedPayment.CardToken))
                {
                    ...
                    if (paymentServiceResponse.ServiceProviderResult.Success && !string.IsNullOrEmpty(paymentServiceResponse.Result))
                    {
                        var payments = new List<PaymentInfo>();
                        payments.Add(paymentServiceResponse.ServiceProviderResult.TokenizedCard);
                        cartResult.Payment = payments.AsReadOnly();

                        var parties = new List<CommerceParty> { paymentServiceResponse.ServiceProviderResult.BillingAddress };
                        cartResult.Parties = parties.AsReadOnly();
                    }
                    else
                    {
                        cartResult.SetErrors(new List<string> { StorefrontManager.GetSystemMessage(StorefrontConstants.SystemMessages.CardAuthorizationFailed) });
                        cartResult.SetErrors(paymentServiceResponse.ServiceProviderResult);
                    }
                }
