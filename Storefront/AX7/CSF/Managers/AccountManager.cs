...
namespace Sitecore.Reference.Storefront.Managers
{
    ...
    public class AccountManager : BaseManager
    {
        ...
        public virtual bool Login([NotNull] CommerceStorefront storefront, [NotNull] VisitorContext visitorContext, string userName, bool persistent)
        {
            Assert.ArgumentNotNullOrEmpty(userName, "userName");            

            var anonymousVisitorCart = this.CartManager.GetCurrentCart(storefront, visitorContext).Result;

            var isLoggedIn = AuthenticationManager.Login(userName, persistent);
            if (!isLoggedIn)
            {
                return false;
            }
        ...
        public virtual ManagerResponse<EnableCustomerResult, CommerceCustomer> FinalizeLinkToExistingCustomer(string emailOfExistingCustomer, string activationCode)
        {
            ...
            var customer = new CommerceCustomer() { Name = emailOfExistingCustomer };
            var request = new EnableCustomerRequest(customer);
            request.Comment = "Finalize";
            request.Properties.Add("ActivationCode", activationCode);
            var result = this.CustomerServiceProvider.EnableCustomer(request);
            return new ManagerResponse<EnableCustomerResult, CommerceCustomer>(result, result.CommerceCustomer);
        }
